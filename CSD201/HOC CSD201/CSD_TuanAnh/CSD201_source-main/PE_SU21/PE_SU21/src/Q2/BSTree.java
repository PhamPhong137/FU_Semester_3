/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package Q2;

/**
 *
 * @author ADMIN
 */
public class BSTree {

    Node root;

    public BSTree() {
    }

    boolean isEmpty() {
        return root == null;
    }

    void insert(String xPlace, int xDepth, int xType) {
        if ((xPlace.charAt(0) == 'B')) {
            return;
        } else {
            Castor value = new Castor(xPlace, xDepth, xType);
            Node node = new Node(value);
            if (isEmpty()) {
                root = node;
                return;
            }
            Node cu = root;
            Node father = null;
            while (cu != null) {
                if (cu.value.depth == value.depth) {
                    System.out.println("Khong the add " + value + " vao trong tree");
                    return;
                }
                father = cu;
                if (cu.value.depth < value.depth) {
                    cu = cu.right;
                } else {
                    cu = cu.left;
                }
            }
            if (father.value.depth < value.depth) {
                father.right = node;
            } else {
                father.left = node;
            }
        }
    }

    void visit(Node p) {
        System.out.print(p.value);
    }

    void preOrder(Node p) {
        if (p == null) {
            return;
        }
        visit(p);
        preOrder(p.left);
        preOrder(p.right);

    }

    void preOrder() {
        preOrder(root);
    }
    int count = 0, i = 0;
    Node node = new Node();

    void postOrder(Node p) {
        if (p == null) {
            return;
        }
        postOrder(p.left);
        postOrder(p.right);
        visit(p);
    }

    void postOrder() {
        postOrder(root);
    }

    void postOrder2(Node p) {
        if (p == null) {
            return;
        }
        postOrder2(p.left);
        postOrder2(p.right);
        if (p.value.type < 7) {
            visit(p);
        }

    }

    void postOrder2() {
        postOrder2(root);
    }

    void InOrder(Node p) {
        if (p == null) {
            return;
        }
        InOrder(p.left);
        visit(p);
        InOrder(p.right);
    }

    void InOrder() {
        InOrder(root);
    }

    void BreadthFirstOrder() {
        BreadthFirstOrder(root);
    }

    void BreadthFirstOrder(Node p) {
        MyQueue queue = new MyQueue();
        if (isEmpty()) {
            return;
        }
        queue.Enqueue(root);
        while (!queue.isEmpty()) {
            Node node = queue.Dequeue();
            if (node.left != null) {
                queue.Enqueue(node.left);
            }
            if (node.right != null) {
                queue.Enqueue(node.right);
            }
            visit(node);
        }
    }

    void deleteDepMax() {
        deleteDepMax(root);
    }

    void deleteDepMax(Node p) {
        int i = 0;
        MyQueue queue = new MyQueue();
        if (isEmpty()) {
            return;
        }
        queue.Enqueue(root);
        while (!queue.isEmpty()) {
            Node node = queue.Dequeue();
            if (node.left != null && i < 2) {
                i++;
                Node depthMax = node;
                if (i == 2) {
                    MyQueue queue1 = new MyQueue();
                    if (isEmpty()) {
                        return;
                    }
                    queue1.Enqueue(node);
                    while (!queue1.isEmpty()) {
                        Node node1 = queue1.Dequeue();
                        if (node1.value.depth > depthMax.value.depth) {
                            depthMax = node1;
                        }
                        if (node1.left != null) {
                            queue1.Enqueue(node1.left);
                        }
                        if (node1.right != null) {
                            queue1.Enqueue(node1.right);
                        }
                    }
                    deleteByCopyL(depthMax);
                }
            }
            if (node.left != null) {
                queue.Enqueue(node.left);
            }
            if (node.right != null) {
                queue.Enqueue(node.right);
            }
        }
    }

    void rotateNodeTh2() {
        int i = 0;
        MyQueue queue = new MyQueue();
        if (isEmpty()) {
            return;
        }
        queue.Enqueue(root);
        while (!queue.isEmpty()) {
            Node node = queue.Dequeue();
            if (node.left != null && i < 2) {
                i++;
                if (i == 2) {
                    if (node == null) {
                        System.out.println("Khong ton tai node p");
                    } else {
                        Node father = getParent(node);
                        Node after = rotateRight(node);//quay phai
                        if (father == null) {
                            root = after;
                        } else {
                            if (father.value.depth < node.value.depth) {
                                father.right = after;
                            } else {
                                father.left = after;
                            }
                        }
                    }
                }
            }
            if (node.left != null) {
                queue.Enqueue(node.left);
            }
            if (node.right != null) {
                queue.Enqueue(node.right);
            }
        }
    }

    void deleteByCopyL(Node p) {
        if (p == null) {
            return;
        }
        if (p.left == null) {
            return;
        }
        if (p.left.right == null) {
            p.value = p.left.value;
            p.left = p.left.left;
            return;
        } else {
            Node father = p.left;
            Node cu = p.left.right;
            while (father.right.right != null) {
                father = father.right;
            }
            p.value = father.right.value;

            if (father.right.left == null) {
                father.right = null;
                return;
            }
            father.right = father.right.left;
        }
    }

    void deleteByCopyR(Node p) {
        if (p == null) {
            return;
        }
        if (p.right == null) {
            return;
        }
        if (p.right.left == null) {
            p.value = p.right.value;
            p.right = p.right.right;
            return;
        } else {
            Node father = p.right;
            Node cu = p.right.left;
            while (father.left.left != null) {
                father = father.left;
            }
            p.value = father.left.value;

            if (father.left.right == null) {
                father.left = null;
                return;
            }
            father.left = father.left.right;
        }
    }

    Node getParent(Node p) {
        if (p == root) {
            return null;
        }
        Node father = null, cu = root;
        while (cu != null && cu.value.depth != p.value.depth) {
            father = cu;
            if (cu.value.depth < p.value.depth) {
                cu = cu.right;
            } else {
                cu = cu.left;
            }
        }
        if (cu == null) {
            return null;
        }
        return father;
    }

    Node findNode(int key) {
        Node cu = root;
        while (cu != null) {
            if (cu.value.depth == key) {
                return cu;
            }
            cu = cu.value.depth < key ? cu.right : cu.left;
        }
        return null;
    }

    void deleteMerging(Node p) {
        Node father = getParent(p);
        if (father == null) {
            if (p.value.depth != root.value.depth) {
                System.out.println("Khong ton tai node p");
                return;
            }
            if (root.left == null) {
                root = root.right;
                return;
            }
            if (root.left.right == null) {
                root.left.right = root.right;
                root = root.left;
                return;
            }
            Node q = root.left;
            while (q.right != null) {
                q = q.right;
            }
            q.right = p.right;
            root = p.left;
            return;
        }
        Castor c = p.value;
        if (p.left == null) {
            if (c.depth < father.value.depth) {
                father.left = p.right;
            } else {
                father.right = p.right;
            }
            return;
        }
        Node q = p.left;
        while (q.right != null) {
            q = q.right;
        }
        q.right = p.right;
        if (c.depth < father.value.depth) {
            father.left = p.left;
        } else {
            father.right = p.left;
        }
    }

    Node rotateRight(Node p) {
        if (p == null || p.left == null) {
            return null;
        }
        Node c = p.left;
        p.left = c.right;
        c.right = p;
        return c;
    }

    Node rotateLeft(Node p) {
        if (p == null || p.right == null) {
            return null;
        }
        Node child = p.right;
        p.right = child.left;
        child.left = p;
        return child;
    }
}
